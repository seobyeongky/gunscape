...
float Fire::Shot(Game_Manager* gm_, Unit* User_, int team_, const coord_def& start_, const coord_def& pos_, float focus_)
{
	...
	if(UseBullet(1))
	{
		float focus2_ = rand_float(-focus_, focus_,"Fire::Shot focus");
		float speed2_ = shot_speed*GetShotSpeedApply()*rand_float(0.8f, 1.2f,"Fire::Shot speed");
...
void Fire::PlayReloadSE(Game_Manager * gm_)
{
